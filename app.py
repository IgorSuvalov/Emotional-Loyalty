...
with tab_manual:
    ...
    Trans_spend = mid_col.slider('"Transactional Spenders"', min_value=-20, max_value=20, value=-5, step=1,
                                 format="%d%%")
    ...
    if st.button("Run the model"):
        if tier_err != 0:
            st.warning("Make sure the values are in the correct ranges first!")
...
with tab_agent:
    ...
    if user_input:
        ...
        if output.get("intent") == "SUGGEST_AND_RUN":
            ...
            st.json({
                "Suggested Parameters": {
                    ...
                    "tier_mix": params.tier_mix
                }
            })

            final_df, _ = run_loyalty_model(
                df,
                {"spend": params.spend, "engage": params.engage},
                params.lam,
                params.multipliers,
                params.tier_mix
            )
